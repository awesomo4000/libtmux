...
pub fn runChecked(
    ...
    var result = try run(allocator, io, tmux_bin, base_args, args);
    if (!result.success()) {
        result.deinit();
...
test "run tmux --version" {
    ...
    // Find tmux first
    const tmux_bin = @import("which.zig").which(allocator, std.testing.io, std.testing.environ, "tmux") catch null;
    if (tmux_bin == null) return; // skip if tmux not installed
    defer allocator.free(tmux_bin.?);

    var result = try run(allocator, std.testing.io, tmux_bin.?, &.{}, &.{"-V"});
    defer result.deinit();
...
test "run tmux with bad command returns non-zero exit" {
    ...
    var result = try run(allocator, std.testing.io, tmux_bin.?, &.{}, &.{"not-a-real-command-xyz"});
    defer result.deinit();
...
test "runChecked tmux version" {
    ...
    const stdout = try runChecked(allocator, std.testing.io, tmux_bin.?, &.{}, &.{"-V"});
...
test "runChecked returns error on failure" {
    ...
    const result = runChecked(allocator, std.testing.io, tmux_bin.?, &.{}, &.{"not-a-real-command-xyz"});
...
test "run with base_args" {
    ...
    var result = try run(allocator, std.testing.io, tmux_bin.?, &.{ "-L", "libtmux-test-nonexistent" }, &.{"-V"});
    defer result.deinit();
